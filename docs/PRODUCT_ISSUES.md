...
- **Decision:** Do not clone Codex pixel-for-pixel. Use it as a quality bar for calm hierarchy, readable measure, alignment, density, and typographic consistency while retaining Onyx’s restrained near-black identity. Use one five-role type scale across the main workspace, 15 pt for composer and response prose, and 12.5 pt for routine activity and waiting text. Keep the readable line length, bias unused width to the trailing side, and place assistant prose, Plan, routine activity, and waiting copy on one conversation axis with disclosure glyphs in an outboard gutter. Plan progress belongs inline; do not restore a detached far-right status column. Markdown lists use a true hanging indent and strong emphasis uses semibold rather than full bold.
- **Acceptance:** The project list, transcript, composer, and inspector share a coherent type scale and spacing rhythm; the conversation is the obvious focal point; transcript and composer share a visibly tighter left axis without crowding compact windows; Plan and activity rows no longer form competing indentation or status columns; wrapped list text aligns beneath its content; controls align cleanly at common window sizes; light and dark appearances avoid muddy or competing surfaces.
- **Verification:** Focused hosted coverage confirms one transcript content axis for assistant, Plan, and routine activity; an outboard disclosure gutter; 15 pt composer/prose; 12.5 pt activity/waiting text; hidden visual Plan status with retained accessibility and tint; semibold Markdown emphasis; hanging list indentation; and a shared 760 pt reading measure. All 60 focused transcript, layout, and composer regressions pass. Fresh same-revision dark, light, narrow, and standalone-transcript renders confirm the intended hierarchy and alignment without changing the near-black baseline. Running-preview validation with representative short and large tasks is still pending.
