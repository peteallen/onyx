...
- **Decision:** Group provider-scoped tasks under an app-owned project catalog. Support add/import, rename, reorder, and metadata-only removal; removing a project grouping must not delete its folder or provider tasks. Label project creation with the visible text “Add Project” and a folder-specific icon so it cannot be mistaken for New Task.
- **Acceptance:** Projects can be added, renamed, moved, and removed from the sidebar; the visible Add Project control reads as a project action rather than another new-task button; tasks appear in the correct project even when providers reuse task IDs; ordering persists; failures remain local to the initiating window.
- **Verification:** Automated catalog and sidebar coverage exercises grouping, ordering, persistence, provider-scoped identity, large catalogs, and project-header interaction. The Add Project control now uses visible folder-specific text and icon, and loading state coverage protects project-metadata-only refreshes. Running-preview management checks, especially the Add Project control, removal wording, and recovery, are still pending.
...
- **Decision:** Do not clone Codex pixel-for-pixel. Use it as a quality bar for calm hierarchy, alignment, density, and typographic consistency while retaining Onyx’s restrained near-black identity. Let chat history and the composer shells fill the conversation pane between modest matching side gutters instead of presenting them as narrow centered cards. Cap assistant prose and entered text at a generous 880 pt measure in unusually wide panes, while expanded activity, its media, and the composer hit surface may use the wider row. Standardize composer input, response prose, waiting text, and routine activity around a 13.5 pt conversation size—slightly larger than the previous tool-use text—and preserve hierarchy through weight and color. Place assistant prose, Plan, routine activity, and waiting copy on one conversation axis with disclosure glyphs in an outboard gutter. Plan progress belongs inline; do not restore a detached far-right status column. Markdown lists use a true hanging indent and strong emphasis uses semibold rather than full bold.
...
- **Verification:** Launch-configuration tests cover path validation, credential/endpoint scrubbing without breaking ordinary toolchain variables, restrictive state-directory creation (including macOS path aliases), and development-only overrides. The native packaging check verifies the exact v0.149.0 archive, complete package layout and metadata, executable architecture, stable identifiers for all four bundled executables, preserved runtime entitlements, nested and outer signatures, and missing/corrupt-artifact failure. It then starts the packaged helper from its final bundle location: a clean-home integration test confirms the exact reported Onyx home, signed-out account, empty active and archived task lists, and untouched inherited Codex home/SQLite/rollout sentinels. Release verification additionally requires the exact two-entry DMG payload, initializes the helper from its mounted package against a fresh private data folder, Gatekeeper-assesses the mounted app when notarization is required, and publishes the verified DMG/checksum as one rollback-protected pair. On 2026-08-23, 25 consecutive helper initialization probes, exact identifier checks for both architecture packages, a wrong-identifier rejection check, and a complete native DMG/checksum round trip passed without launching Onyx. A universal release artifact, clean-machine OAuth, and cross-app rename/archive/delete/sign-out/relaunch checks are still pending, so this remains **In progress**.
...
- **Status:** Active
- **Decision:** Keep `codex app-server`, but ship and pin the helper inside Onyx and give it an Onyx-owned `CODEX_HOME`. Never reuse the official Codex home or credentials, and never silently fall back to an installed binary in production.
...
- **Verification:** Runtime launch-configuration, native bundled-package validation, and initialization-home readback from a mounted native DMG pass. Universal release verification, clean-machine OAuth, and cross-app history isolation remain required before marking RUNTIME-003 **Done**.
