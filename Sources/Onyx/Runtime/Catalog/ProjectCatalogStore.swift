...
actor ProjectCatalogStore {
    ...
    /// Imports project metadata already embedded in conversation records. This
    /// is the compatibility bridge for catalogs created before projects had
    /// independent IDs and ordering. The complete legacy conversation catalog
    /// must be supplied on the first call. Completion is persisted atomically,
    /// including when no projects exist, so later calls cannot resurrect a
    /// project the user removed from Onyx.
    @discardableResult
    func importConversationProjects(
        from conversations: [ConversationCatalogRecord]
    ) throws -> [ProjectCatalogRecord] {
        try importLegacyProjects(from: conversations.compactMap(\.project))
    }

    /// Production migration source for the pre-catalog task list. Codex and
    /// local provider tasks already carry their working folder, even though
    /// the older app-owned conversation catalog was never wired into the
    /// workspace. Treat each distinct working folder as the legacy project
    /// metadata and commit the same one-time marker used above.
    @discardableResult
    func importTaskProjects(
        from threads: [RuntimeThread]
    ) throws -> [ProjectCatalogRecord] {
        try importLegacyProjects(from: threads.compactMap { thread in
            thread.cwd.map { ConversationProject(path: $0) }
        })
    }

    private func importLegacyProjects(
        from legacyProjects: [ConversationProject]
    ) throws -> [ProjectCatalogRecord] {
        try Self.fileAccess.withLock(for: fileURL) {
            let current = try loadFromDisk()
            guard !current.didBootstrapConversationProjects else { return [] }

            var projects = current.projects
            var results: [ProjectCatalogRecord] = []
            var returnedIDs: Set<ProjectID> = []
            let timestamp = now()

            for legacyProject in legacyProjects {
                // Historical conversation metadata was not path-validated.
                // Skip unusable legacy rows so one malformed value cannot
                // block valid imports or force the migration to retry forever.
                guard let normalizedPath = ProjectPathNormalizer.normalize(
                    legacyProject.path
                ) else { continue }
                let resolved: ProjectCatalogRecord
                if let existing = projects.first(where: {
                    $0.folderPath == normalizedPath
                }) {
                    resolved = existing
                } else {
                    let generatedID = Self.uniqueID(excluding: Set(projects.map(\.id)))
                    resolved = ProjectCatalogRecord(
                        id: generatedID,
                        folderPath: normalizedPath,
                        displayName: Self.importDisplayName(
                ...
                if returnedIDs.insert(resolved.id).inserted {
                    results.append(resolved)
                }
            }

            // Always persist, even when both collections are empty: the marker
            // is what makes this migration one-time and removal authoritative.
            try persistValidated(
                projects,
