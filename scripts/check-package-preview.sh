...
[[ "$script_text" == *'preview_source_revision="$(/usr/bin/git -C "$repo_root" rev-parse HEAD 2>/dev/null)" || {'* ]]
[[ "$script_text" == *'repository HEAD is not a full commit SHA'* ]]
[[ "$script_text" == *'assert_source_clean()'* ]]
[[ "$script_text" == *'git -C "$repo_root" diff-index --quiet HEAD --'* ]]
[[ "$script_text" == *'git -C "$repo_root" ls-files --others --exclude-standard'* ]]
[[ "$script_text" == *'[[ "$current_revision" == "$preview_source_revision" ]]'* ]]
[[ "$script_text" == *'repository HEAD changed while packaging'* ]]
[[ "$script_text" == *'tracked build inputs changed'* ]]
[[ "$script_text" == *'untracked build inputs would make the embedded revision inaccurate'* ]]
...
[[ "$script_text" == *'--source-revision "$preview_source_revision"'* ]]
[[ "$script_text" == *'bundle_source_revision="$(/usr/bin/plutil -extract OnyxSourceRevision raw -o -'* ]]
[[ "$script_text" == *'packaged preview source revision is missing'* ]]
[[ "$script_text" == *'packaged preview source revision changed'* ]]
[[ "$script_text" == *$'[[ ! -L "$preview_app" && ! -L "$preview_executable" ]] || \\\n  die "refusing to operate through a symbolic link: $preview_app"\n\nassert_source_clean'* ]]
